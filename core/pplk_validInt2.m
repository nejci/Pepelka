function [validInt, list] = pplk_validInt2(data, labels, methods, options)
% PPLK_VALIDINT 
...
if groupModeAny
    ...
    if groupModeDNmod
        ...
        if isempty(graph)
            ...
            [graph,~,uniqueInd] = graph_create2(data,[],graph_type,graphOpt);
            chdir(oldDir);
    ...
    for k = NCind
        ...
        if groupModeXB
            ...
            %[XBdummy, XBmoddummy] = indexXB2(data, labels(:,k));
            chdir(oldDir);
        end
    end
    ...
    switch(currMethod)
        ...
        case {'DNS'}            
            ...
            for k = NCind
                ...
                [~,~,labelsNewRepaired] = unique(labelsGraphNew(:,k));
                % Old version
                %DNS(k) = indexDNs_graph(graph,labelsNewRepaired,0);
                DNS(k) = indexDNs_graph2(graph,graphShortPaths,dataGraphNew,labelsNewRepaired,isDirected,graph_type,graphOpt,penalFun,penalFunP,penalFunLogMid);
