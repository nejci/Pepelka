...
% CHANGED by Nejc!  som_kmeans replaced with MATLAB kmeans, which seems to
% work more correct.
%
...
if verbose, fprintf(1,'Doing k-means for 2-%d clusters\n',n_max); end
...
for i = 2:n_max, % number of clusters
    ...
    % 	[k,c,errV]=kmeans(D,i, 'distance','sqEuclidean','maxiter',100, ...
    % 	   'start','sample','replicates', 1, 'emptyaction','singleton',...
    % 	   'display','notify');
    %     err=sum(errV);
    % 	k=k';
    %
    ...
    if verbose,
        fprintf(1,'%d/%d clusters (%d runs)\n', i, n_max, c_max);
    end
    [k_best,c_best,errV] = kmeans(D, i, 'distance','sqEuclidean','maxiter',100, ...
        'start','sample','replicates', c_max, 'emptyaction','singleton',...
        'display','notify');
    best = sum(errV);
    % store the results
    centers{i}  = c_best;
    ...
    ind(i)      = db_index(D, k_best, c_best,2,2); % modified by kr 1.10.02
