function [sMap,sTrain] = som_batchtrain_dotprodc(sMap, D, varargin)
% Uncentered Pearson Correlation used as similarity metric!!!
% NOTES:
%		- in gSOM.m, data is normalized on interval [0,1], only when Squared Euclidean
% distance is used. Correlation implicitely normalizes data on zero mean
% and unit std.
%
...
% varargin
sTrain = som_set('som_train','algorithm','batch','distfun','correlation','distmode','dotprod','neigh', ...
...
%debug mode
% trainlen = 4000;
% r0 = 3;%sTrain.radius_ini; 
% r1 = 1;%sTrain.radius_fin;
% radius = r1 + fliplr((0:(trainlen-1))/(trainlen-1)) * (r0 - r1);


% NO CHANGES:
% distance between map units in the output space - Euclidean distance,
% since Pearson Correlation makes no sense here (C([0,0], [X,X])=NaN).
%
...
for t = 1:trainlen,  
  ...
  while i0+1<=dlen, 
	...
	% transforming portion of data points
	...
	[ddists(inds), bmus(inds)] = max(D_dist);
  end
  
  % tracking
  if tracking > 0,
    ddists = ddists+1; % add the constant term to scale distance form [-1,1] to [0,2]
...
%   tic
%   M_norm = sqrt(diag(M * M'));
%   M = M ./ repmat(M_norm,1,size(M,2));
%   t2=toc;
%   disp(['Matrika: ', num2str(t2),' s']);
  
  % Normalization of M (this loop is usualy faster than matrix op.)
  for ind=1:size(M,1)
	  datum = M(ind,:) - mean(M(ind,:));
	  M(ind,:)= datum./norm(datum);
  end
...
% update structures
%sTrain = som_set(sTrain,'bmus',bmus,'time',datestr(now,0));
