...
stream1 = RandStream('mt19937ar','Seed',sum(100*clock));
...
for t = 1:trainlen, 
  ...
  % more memory.
  ind = rem(t,update_step); 
  if ind==0, ind = update_step; end
  
  if ind==1, 
    steps = [t:min(trainlen,t+update_step-1)];
    % sample order    
    switch sample_order_type, 
     case 'ordered', samples = rem(steps-1,dlen)+1;
    ...
    switch sTrain.alpha_type,
     case 'linear',       alfa = (1-(steps-1)/trainlen)*alpha;
