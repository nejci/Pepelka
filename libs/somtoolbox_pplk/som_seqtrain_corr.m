function [sMap, sTrain] = som_seqtrain_corr(sMap, D, varargin)
...
% varargin
sTrain = som_set('som_train','algorithm','seq','distfun','correlation','distmode','euc','neigh', ...
...
for t = 1:trainlen, 
  ...
  %compute Pearson correlation between each map unit and sample x
  xt = x - mean(x);
  xnorm = sqrt(sum(xt.^2));
  xt = xt ./ xnorm;
