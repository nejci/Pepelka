function [sMap, sTrain] = som_seqtrain_dotprod(sMap, D, varargin)
...
% varargin
sTrain = som_set('som_train','algorithm','seq','distfun','ucorrelation','distmode','dotprod','neigh', ...
...
for t = 1:trainlen, 
  ...
  %compute dot product (uncentered P. correlation) between each map unit and sample x
   
  %xt = x ./norm(x); % if we want to normalize the input vector
  xt=x;	%if not - there is difference however
  x_dist = M * xt';
