function [sMap, sTrain] = som_seqtrain_dotprodc(sMap, D, varargin)
% Dot-product SOM for centered data = Pearson Correlation
...
% varargin
sTrain = som_set('som_train','algorithm','seq','distfun','correlation','distmode','dotprod','neigh', ...
...
%debug mode - for comparisson with MeV package
debug=0;
if debug==1
	update_step=1;
	sample_order_type='ordered';
	rini=3;
	radius = [rini 1]; 
	rstep = (radius(end)-radius(1))/(trainlen-1);
	radius_type='linear';
...
for t = 1:trainlen
  ...
  if tracking > 1 && ind==update_step
	plot(D','-','color',[0.7 0.7 0.7]); hold on;
  end
  ...
  if ind==1, 
    ...
    switch sample_order_type, 
     case 'ordered', samples = rem(steps-debug,dlen)+1;
     case 'random',  samples = ceil(dlen*rand(update_step,1)+eps);
    end

    % neighborhood radius - debug mode
    switch radius_type, 
     case 'linear'
		 if(debug==1)
			r = 1 + (rini-1)*(trainlen-t+1)/trainlen;
		 else
			r = rini+(steps-1)*rstep;
		 end
    ...
    % learning rate - debug mode
    switch sTrain.alpha_type,
     case 'linear'
		 if (debug==1)
			 alfa = alpha* (trainlen-t+1)/trainlen;
		 else
			 alfa = (1-steps/trainlen)*alpha;
		 end
  ...
  %compute dot product (centered P. correlation) between each map unit
  %and sample x
  % We assume, that x has already been centered (x - mean(x)) and
  % normalized (x ./ norm(x))
  xt=x;	
  x_dist = M * xt';
 
  [qerr bmu] = max(x_dist); % maximum value (cosine) and the BMU
  ...
  % update M
  nom = M(:,known) + h(:,ones(sum(known),1)) .* xt(mu_x_1,known);
  % center nom (mean subtraction)
  nom = bsxfun(@minus,nom,mean(nom,2));
  % normalization
  dnom = sqrt(sum(nom.^2,2)); % norm(nom)
  M(:,known) = nom ./ repmat(dnom,1,size(M(:,known),2));
