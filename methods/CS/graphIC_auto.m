...
%               - dodano avtomatsko ocenjevanje sigme
...
%
% INPUT
% data: Data set to be clustered (dim x patterns)
...
function [l,K_true,IC,l_h]=graphIC_auto(data,Kf,Kin,Nin,draw);

[dim,n]=size(data);

%Silverman's rule of a thumb for sigma estimation
sig=estimate_sigma(data');
...
if(draw & dim==2)
   ...
   % Seed the Kin clusters; return l (labels) and M (means) 
    [l,ul,KM]=seed(data,sig,Kin,Nin,fig2);

else
    % Seed the Kin clusters; return l (labels) and M (means) 
    [l,ul,KM]=seed(data,sig,Kin,Nin,0);
end
...
[VOL,CUT]=cut_vol(l,data,sig,Kin);
