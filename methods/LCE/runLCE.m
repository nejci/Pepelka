function [labelsCE,numClust]=runLCE(E, K, methodSim, methodCons, dc, iter)
%==========================================================================
...
%						('CTS','SRS','ASRS')
%			methodCons = name(s) of the method for final clustering
...
%
% OUTPUTS: labelsCE = an N-by-|methodCons| vector of clustering result 
%==========================================================================
...
if ~all(ismember(methodCons,{'SL', 'CL', 'AL'}))
...
numConsFun = 1;
if iscell(methodCons)
    numConsFun = length(methodCons);
else
    methodCons = {methodCons};
end
labelsCE = zeros(size(E,1),numConsFun);
numClust = zeros(1,numConsFun);

for c = 1:numConsFun
    labelsCE(:,c) = consFuncLCE(S,K,methodCons{c});
    numClust(c) = length(unique(labelsCE(:,c)));
end
