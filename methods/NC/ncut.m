...
%     dataNcut.eigsErrorTolerance = 1e-8; error tolerance in eigensolver
...
[vbar,s] = eigs(@mex_w_times_x_symmetric,size(P,1),nbEigenValues,'LA',options,tril(P)); % FIX when migrating from M2012a
%warning on
...
% ADDED BY NEJC - for robustness
zeroRowInd = all(bsxfun(@eq,Eigenvectors,zeros(1,size(Eigenvectors,2))),2);
if any(zeroRowInd)
    Eigenvectors(zeroRowInd,:) = Eigenvectors(zeroRowInd,:)+eps;
end
