...
%     end
    ...
    switch type
        case 'Isomap'         
            ...
            numNN = min(12,size(A,1)-1);
			if isempty(varargin), [mappedA, mapping] = isomap(A, no_dims, numNN);
            else [mappedA, mapping] = isomap(A, no_dims, varargin{1}); end
            mapping.name = 'Isomap';
            if size(mappedA,1) ~= size(A,1)
                warning('pplk:reduce','Output number of mapped elements is less than input number. Try to increase number of nearest neighbors.');
            end
            
		case 'LandmarkIsomap'
			% Compute Landmark Isomap mapping
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = landmark_isomap(A, no_dims, numNN, 0.2);
        ...
        case {'Laplacian', 'LaplacianEig', 'LaplacianEigen' 'LaplacianEigenmaps'}
            ...
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = laplacian_eigen(A, no_dims, numNN, 1, eig_impl);
        ...
        case {'HLLE', 'HessianLLE'}
            ...
            numNN = min(12,size(A,1)-1);
			if isempty(varargin), mappedA = hlle(A, no_dims, numNN, eig_impl);
        ...
        case 'LLE'
            % Compute LLE mapping
            numNN = min(12,size(A,1)-1);
			if isempty(varargin), [mappedA, mapping] = lle(A, no_dims, numNN, eig_impl);
        ...
        case 'LLC'
            % Compute LLC mapping
            numNN = min(12,size(A,1)-1);
			if isempty(varargin), mappedA = run_llc(A', no_dims, numNN, 20, 200, eig_impl);
        ...
        case 'LTSA'
            ...
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), mappedA = ltsa(A, no_dims, numNN, eig_impl);
        ...
        case 'LLTSA'
            ...
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = lltsa(A, no_dims, numNN, eig_impl);
        ...
        case {'LMVU', 'LandmarkMVU'}
            ...
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = lmvu(A', no_dims, 5, numNN);
        ...
        case 'FastMVU'
            % Compute MVU mapping
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = fastmvu(A, no_dims, numNN, eig_impl);
        ...
        case {'Conformal', 'ConformalEig', 'ConformalEigen', 'ConformalEigenmaps', 'CCA', 'MVU'}
            ...
            numNN = min(12,size(A,1)-1);
            tmp_dims = min([numNN, size(A, 2), 4 * no_dims + 1]);
            if isempty(varargin), [mappedA, mapping] = lle(A, tmp_dims, numNN, eig_impl);
        ...
        case 'LPP'
            % Compute LPP mapping
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = lpp(A, no_dims, numNN, 1, eig_impl);
        ...
        case 'NPE'
            % Compute NPE mapping
            numNN = min(12,size(A,1)-1);
            if isempty(varargin), [mappedA, mapping] = npe(A, no_dims, numNN, eig_impl);
