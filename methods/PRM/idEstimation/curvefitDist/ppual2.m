...
if ~isstruct(pp), pp = fn2fm2(pp); end

if iscell(pp.breaks)   % we are dealing with a multivariate spline

   [breaks,coefs,l,k,d] = ppbrk2(pp); m = length(breaks);
   ...
   if iscell(x)  % evaluation on a mesh
      ...
      for i=m:-1:1
         ...
         v = reshape(ppual1(...
              ppmak2(breaks{i},reshape(v,dd*l(i),k(i)),dd), x{i}, left{i} ),...
                [sizev(1:m),nsizev(i)]);
         sizev(m+1) = nsizev(i);
         if m>1
