function [eigvector,eigvalue,data_out] = kernelPCA(data,options)
%KPCA	Kernel Principal Component Analysis
% [eigvector,eigvalue,data_out] = KERNELPCA(data,options)
%
% INPUT:
%   data    : if options.Kernel = 0
%               Data matrix [nSamples X nDimensions]
%             if options.Kernel = 1
%               Kernel matrix.
%
%   options : struct with fields:
%              .Kernel   
%                   1: data is actually the kernel matrix.
%                   0: ordinary data matrix (default).
%
%               .KernelType
...
%                   'Linear':       x'*y
%
%               .t                  parameter for Gaussian kernel
%               .d                  parameter for Poly kernel
%               .ReducedDim 
%                   The dimensionality of the reduced subspace. 
%                   If 0, all the dimensions will be kept. Default is 30.
%
% OUTPUT:
...
%   data_out  : projected data onto options.ReducedDim dimensions
%
% EXAMPLE:
...
%   
%   dataTest = rand(3,10);
%   Ktest = constructKernel(dataTest,data,options)
%   Y = Ktest*eigvector;
%
% REFERENCE:
...
data_out = eigvector(:,1:ReducedDim)'*K';
data_out = data_out';
