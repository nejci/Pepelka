function [sMap, sTrain] = som_seqtrain_ucorr(sMap, D, varargin)
...
% varargin
sTrain = som_set('som_train','algorithm','seq','distfun','ucorrelation','distmode','euc','neigh', ...
...
for t = 1:trainlen, 
  ...
  %compute uncentered Pearson correlation between each map unit and sample x
  xt = x;
  xnorm = sqrt(sum(xt.^2));
  xt = xt ./ xnorm;
  ...
  x_dist = Mt * xt';
  x_dist = (-1) * x_dist; % simmilarity -> distance (high correlation -> small distance)
  
  [qerr bmu] = min(x_dist); % minimum distance and the BMU
  ...
  % update M
  % BE CAREFUL - this adaptation rule is derived for Euclidean distance
