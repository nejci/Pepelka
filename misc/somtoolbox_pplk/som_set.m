  ...
  switch sS
   ...
   case 'som_data', 
    sS=struct('type', 'som_data', ...
              ...
              'comp_norm', cell(1), ...
              'label_names', [], ...
              'pca_vec',[], ... %dodal Nejc Ilc
              'pca_val',[]);    %dodal Nejc Ilc
   case 'som_topol',
   ...
   case 'som_train',
    sS=struct('type', 'som_train', ...
              'algorithm', '', ...
              'data_name', '', ...
			  'data_new', [], ... % added by Nejc Ilc
			  'bmus', [], ...     % added by Nejc Ilc
              'neigh', 'gaussian', ...
              'mask', [], ...
              'radius_ini', NaN, ...
              'radius_fin', NaN, ...
              ...
              'trainlen', NaN, ...
              'time', '', ...
			  'distfun','',... % added by Nejc Ilc
			  'distmode',''); % added by Nejc Ilc
   case 'som_norm',
  ...
  switch sS.type, 
   ...
   case 'som_train', 
	...
	sS.algorithm = content; isok = 1;
	  ...
	  case 'distfun', 
      if ~ischar(content),
	msg = '''distfun'' should be a string.'; 
      else
	sS.distfun = content; isok = 1;
	  end
	  case 'distmode', 
      if ~ischar(content),
	msg = '''distmode'' should be a string.'; 
      else
	sS.distmode = content; isok = 1;
      end
     case 'data_name', 
      if ~ischar(content),
	...
	sS.data_name = content; isok = 1;
	  ...
	  case 'data_new', 
      if ~isnumeric(content),
	msg = '''data_new'' should be a matrix'; 
      else
	sS.data_new = content; isok = 1;
	  end
	  % added by Nejc Ilc
	  case 'bmus', 
      if ~isnumeric(content),
	msg = '''bmus'' should be a vector'; 
      else
	sS.bmus = content; isok = 1;
      end
     case 'neigh', 
      if ~ischar(content),
	msg = '''neigh'' should be a string'; 
