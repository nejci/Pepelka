...
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
	...
	sTopol.msize = varargin{i}.msize; 
     ...
     %dodal Nejc Ilc
     case 'pca_val', i=i+1; pca_val=varargin{i};
...
elseif munits < 0,  %dodal Nejc Ilc - pomeni, da imamo kot velikost podan faktor mnozenja
    if ~isnan(dlen), 
        munits =abs(munits)* ceil(5 * dlen^0.5); % this is just one way to make a guess...
    else
        munits = 100; % just a convenient value
    end
end
...
% dodal Nejc Ilc - dela hitreje in stabilneje
  %[V,S,L]=princomp(D,'econ');
  %eigval  = L(end:-1:1);
% vrednost dobimo kar preko argumenta, izracuna se v som_data_struct.m
  
 
  eigval=pca_val(end:-1:1);
  ...
  %Nejc Ilc - tule je bil bug -> ni se upostevala dejanska topologija
